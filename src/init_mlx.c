...
void	check_img(t_game *game)
{
	if (!game->st_mlx->img_f_1)
		close_window(game, 1, "image load failed\n");
...
void	init_mlx(t_game *game)
{
	...
	if (!game->st_mlx)
		close_window(game, 1, "mlx load failed\n");
	...
	game->st_mlx->win_ptr = mlx_new_window(game->st_mlx->mlx_ptr,
			game->st_map->size_c * 128, game->st_map->size_l * 128, "so_long");
	if (!game->st_mlx->win_ptr)
		close_window(game, 1, "mlx win load failed\n");
}
